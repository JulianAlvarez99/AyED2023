...
graph* graph_new(); // Create a new graph
...
int graph_add_edge_byname(graph* g, t_graph_elem v1, t_graph_elem v2, int weight, int cmp (t_graph_elem, t_graph_elem));// Add an edge to the graph by name
...
int graph_is_edge (graph* g, int v1, int v2); // Return 1 if exists an edge between v1 and v2, 0 otherwise
int graph_vertex_count(graph* g); // Get the vertex count
int graph_vertex_degree(graph* g, int v); // Get the degree of a vertex (7.2.a)
//void graph_vertex_adjacent_list(graph* g, int v, list* L);// Get ayencency list of a vertex (7.2.b)
void graph_bfs(graph* g, int v, int visit(t_graph_elem, int, void*), void* ctx);// Search Breadth First Search
void graph_dfs(graph* g, int v, int visit(t_graph_elem, int, void*), void* ctx);// Search Depth First Search
